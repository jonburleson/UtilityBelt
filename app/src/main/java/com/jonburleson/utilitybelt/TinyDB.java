...
public class TinyDB {
    ...
    /**
     * Custom put and get for ListObject of type Note
     */
    public void putListNote(String key, ArrayList<Note> noteList){
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<>();
        for(Note n: noteList){
            objStrings.add(gson.toJson(n));
        }
        putListString(key, objStrings);
    }

    public ArrayList<Note> getListNote(String key){
        Gson gson = new Gson();
